...
<div class="space-y-6">
    ...
    <form action="{{ route('cms.events.update', $event) }}" method="POST" enctype="multipart/form-data" class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-6">
        ...
        <div>
            ...
            <textarea id="summary" name="summary" class="mt-2 block w-full rounded-md border-0 py-2.5 px-3 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6 dark:bg-gray-700 dark:text-white dark:ring-gray-600" rows="4">{{ old('summary', $event->summary) }}</textarea>
        </div>
        ...
        <div class="flex justify-between gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
            ...
            <div class="flex gap-3">
                ...
                <button type="submit" name="action" value="publish" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm transition-colors duration-200">Update & Publish</button>
            </div>
        </div>
    </form>
</div>
