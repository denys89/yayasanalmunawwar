...
<div class="flex items-center justify-between mb-6">
    <div class="flex items-center space-x-4">
        ...
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-200">Edit Role: {{ ucwords(str_replace('-', ' ', $role->name)) }}</h1>
    </div>
</div>

<form action="{{ route('cms.roles.update', $role) }}" method="POST">
    @csrf
    @method('PUT')
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="e.g., Content Manager" required>
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Will be converted to slug format</p>
                        @error('name')
                    ...
                    <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            <strong>Created:</strong> {{ $role->created_at?->format('M d, Y H:i') ?? 'N/A' }}
                        </p>
                        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                            <strong>Updated:</strong> {{ $role->updated_at?->format('M d, Y H:i') ?? 'N/A' }}
                        </p>
                    </div>
                </div>
                ...
                <div class="mt-6 space-y-3">
                    ...
                    <a href="{{ route('cms.roles.index') }}" class="w-full inline-flex justify-center items-center px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 rounded-lg transition-colors duration-200">
                        Cancel
                    </a>
                </div>
            </div>
                    ...
                    <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            ...
                            <label class="flex items-center space-x-3 cursor-pointer group">
                                <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" class="permission-checkbox group-{{ Str::slug($group) }} rounded border-gray-300 dark:border-gray-600 text-blue-600 focus:ring-blue-500 dark:bg-gray-700" {{ in_array($permission->id, old('permissions', $rolePermissions)) ? 'checked' : '' }}>
